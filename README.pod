...
Only one channel can be logged at a time. Messages are printed to stdout which are captured by systemd and automatically logged in journald. Only PRIVMSG, NOTICE, PART, JOIN, KICK, MODE, and QUIT messages are logged.
